const realInput = `LHLRlCCvCLVgHPfCHtVjBGrBDNzWFBsBGBfscGsD
nQwbnwwpbrJBrNWB
...
console.log(
  Object.entries(
    input.split("\n").reduce((groups, rucksack, index) => {
      const bagIndex = Math.floor(index / 3);
      groups[bagIndex] = [...(groups[bagIndex] ?? []), rucksack];
      return groups;
    }, {})
  ).reduce((sum, [_, groupItems]) => {
    const comp1 = [...groupItems[0]];
    const comp2 = [...groupItems[1]];
    const comp3 = [...groupItems[2]];
    ...
    return (
      ...
      Array.from(
        comp1
          .filter(
            (comp1Item) =>
              comp2.includes(comp1Item) && comp3.includes(comp1Item)
          )
          .reduce((uniques, item) => {
            uniques.add(item);
            return uniques;
        ...
        const prio = charCode > 96 ? charCode - 96 : charCode - 38;
        return sum + prio;
      }, 0)
    );
  }, 0)
