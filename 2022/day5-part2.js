...
const move = (crates, from, to, amount) => {
  let allCrates = [];
  while (amount--) {
    allCrates.push(crates[from].shift());
  }
  allCrates.reverse().forEach((crate) => crates[to].unshift(crate));
  return crates;
};
...
console.log(
  Object.values(
    instructions.reduce(
      (crates, { from, to, amount }) => move(crates, from, to, amount),
      crates
    )
  )
    .map(([c]) => c)
